...
const Dashboard = () => {
  ...
  const buscarLeads = async () => {
    try {
      ...
      // Filtra apenas leads fechados
      const fechados = dados.filter(lead => lead.Status === 'Fechado');
  ...
  const leadsFechadosCount = leadsFechados.length;
  ...
  // Contadores por seguradora (usando leads fechados)
  ...
  const totalPremioLiquido = leadsFechados.reduce(
    (acc, curr) => acc + (Number(curr.PremioLiquido) || 0),
    0
  );

  // Calcular média ponderada de comissão global
  const somaPonderadaComissao = leadsFechados.reduce((acc, lead) => {
